...
fn (g &Generator) emit_param_field(f Field) string {
	...
	// File parameters take a file_id/HTTP URL here; uploading a new local file
	// is done via Bot.http_request_files instead.
	if f.types.any(it.contains('InputFile')) {
		note = 'file_id or HTTP URL; upload a local file via Bot.http_request_files'
	}
